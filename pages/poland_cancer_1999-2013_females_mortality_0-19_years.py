...
dash.register_page(__name__, path="/poland_cancer_1999-2013_females_mortality0_19", suppress_callback_exceptions=True)
# ---------------------------------------------------------------
...
df = pd.read_excel(
    r"C:\Users\kruge\PycharmProjects\cancer\pages\dataset\Mortality_by_cancer_summary_Poland\Mortality_by_cancer_summary_Poland_0-19_Years_1999_2013.xlsx")
...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            dcc.Graph(id='barchart_6'),
        ], className='six columns'),

        html.Div([
            dcc.Graph(id='piechart_6'),
        ], className='six columns'),
...
# ------------------------------------------------------------------
@callback(
    [Output('piechart_6', 'figure'),
     Output('barchart_6', 'figure')],
...
def update_data(chosen_rows, piedropval, bardropval):
    if len(chosen_rows) == 0:
        df_filtered = dff[dff['Cancer'].isin(
            ['Leukaemia', 'Brain and other CNS', 'Lymphocytic Leukemia', 'Soft tissue', 'Thyroid gland',
             'Bones and joints', 'Kidney',
    ...
    bar_chart = px.bar(
        ...
        title='Mortality by cancer - Poland - 1999-2013 - 20-49 years - Females',  # figure title
        width=2500,  # figure width in pixels
        height=720,  # figure height in pixels

...
dash.register_page(__name__, path="/poland_cancer_1999-2013_males_incidence20_49", suppress_callback_exceptions=True)
# ---------------------------------------------------------------
...
df = pd.read_excel(
    r"C:\Users\kruge\PycharmProjects\cancer\pages\dataset\Incidence_by_cancer_males_Poland\Incidence_by_cancer_males_Poland_20-49_Years_1999_2013.xlsx")
dff = df.groupby('Cancer', as_index=False)[
    ['Number', 'Crude rate', 'ASR (W)', 'ASR (E Old)', 'ASR (E New)']].sum()
print(dff[:5])
# ---------------------------------------------------------------
layout = html.Div([
    html.Div([
    ...
    html.Div([
        html.Div([
            dcc.Graph(id='barchart_13'),
        ], className='six columns'),

        html.Div([
            dcc.Graph(id='piechart_13'),
        ], className='six columns'),
...
# ------------------------------------------------------------------
@callback(
    [Output('piechart_13', 'figure'),
     Output('barchart_13', 'figure')],
...
def update_data(chosen_rows, piedropval, bardropval):
    if len(chosen_rows) == 0:
        df_filtered = dff[dff['Cancer'].isin(
            ['Leukaemia', 'Brain and other CNS', 'Lymphocytic Leukemia', 'Festis', 'Thyroid gland', 'Bones and joints',
             'Kidney',
             'Prostate', 'Non Hodgkin lymphoma', 'Endocrine', 'Lungs', 'Penis'])]
    else:
        print(chosen_rows)
    ...
    bar_chart = px.bar(
        ...
        title='Incidence by cancer - Poland - 1999-2013 - 20-49 years - Males',  # figure title
        width=2500,  # figure width in pixels
        height=720,  # figure height in pixels

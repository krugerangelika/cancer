...
dash.register_page(__name__, path="/poland_cancer_1999-2013_males_mortality0_19", suppress_callback_exceptions=True)
# ---------------------------------------------------------------
...
df = pd.read_excel(
    r"C:\Users\kruge\PycharmProjects\cancer\pages\dataset\Mortality_by_cancer_males_Poland\Mortality_by_cancer_males_Poland_0-19_Years_1999_2013.xlsx")
...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            dcc.Graph(id='barchart_12'),
        ], className='six columns'),

        html.Div([
            dcc.Graph(id='piechart_12'),
        ], className='six columns'),
...
# ------------------------------------------------------------------
@callback(
    [Output('piechart_12', 'figure'),
     Output('barchart_12', 'figure')],
...
def update_data(chosen_rows, piedropval, bardropval):
    ...
    bar_chart = px.bar(
        ...
        title='Mortality by cancer - Poland - 1999-2013 - 0-19 years - Males',  # figure title
        width=2500,  # figure width in pixels
        height=720,  # figure height in pixels

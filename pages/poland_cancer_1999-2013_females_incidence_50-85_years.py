...
dash.register_page(__name__, path="/poland_cancer_1999-2013_females_incidence50_85", suppress_callback_exceptions=True)
# ---------------------------------------------------------------
...
df = pd.read_excel(r"C:\Users\kruge\PycharmProjects\cancer\pages\dataset\Incidence_by_cancer_summary_Poland\Incidence_by_cancer_summary_Poland_50-85_Years_1999_2013.xlsx")
...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            dcc.Graph(id='barchart_8'),
        ], className='six columns'),

        html.Div([
            dcc.Graph(id='piechart_8'),
        ], className='six columns'),
...
# ------------------------------------------------------------------
@callback(
    [Output('piechart_8', 'figure'),
     Output('barchart_8', 'figure')],
...
def update_data(chosen_rows, piedropval, bardropval):
    ...
    bar_chart = px.bar(
        ...
        title='Incidence by cancer - Poland - 1999-2013 - 50-85 years - Females',  # figure title
        width=2500,  # figure width in pixels
        height=720,  # figure height in pixels
